...
const ZoomTo = ({ mapActionConfig, abortPreviousAction = true, delay, setProps }) => {

    // 取得传递的地图实例
    const { current: map } = useMap();

    // 每次mapActionConfig有效时执行zoomTo()动作
    useEffect(() => {
        if (mapActionConfig) {
            // 拆分出zoom参数与其他参数
            let { zoom, ...rest } = mapActionConfig;
...
export default React.memo(ZoomTo);